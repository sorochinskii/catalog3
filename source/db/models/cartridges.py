...
class Model(BaseCommon):
    ...
    printer: Mapped[list['Printer']] = relationship(
        back_populates='cartridge_model', uselist=False)
...
class Cartridge(Device, PolymorphicMixin):
    ...
    # current_printer_id: Mapped[int | None] = mapped_column(
    #     ForeignKey('printer.id'))
    # current_printer: Mapped['Printer'] = relationship(
    #     back_populates='current_cartridge', uselist=False)
